...
export TARGET_NAMESPACE=${1:-"region1"}
export RECOVERY_NAMESPACE=${2:-"region2"}
if [ $# -gt 2 ]
  then
...
export HACRR_ADDRESS=$(kubectl get service secureapphelm-ibm-mq-loadbalancer-hacrr -n $RECOVERY_NAMESPACE -o jsonpath="{.status..hostname}")\(9415\)
export NATIVEHA_GROUP_NAME=qm-live

if [[ ${#HACRR_ADDRESS} -gt 6 ]] ; then
    echo "Found HACRR_ADDRESS of $HACRR_ADDRESS"
else
    echo "Failed to find region2 HA CRR address. Please assure you ran installRegion2.sh"
    exit 1
fi

( echo "cat <<EOF" ; cat mtlsqm.yaml_template ; echo EOF ) | sh > mtlsqm.yaml

( echo "cat <<EOF" ; cat secureapp_nativeha.yaml_template ; echo EOF ) | sh > secureapp_nativeha_region1.yaml

kubectl create namespace $TARGET_NAMESPACE
kubectl config set-context --current --namespace=$TARGET_NAMESPACE
kubectl apply -f mtlsqm.yaml

helm install secureapphelm ../../../charts/ibm-mq -f secureapp_nativeha_region1.yaml $MQ_ADMIN_PASSWORD_NAME $MQ_ADMIN_PASSWORD_VALUE $MQ_APP_PASSWORD_NAME $MQ_APP_PASSWORD_VALUE $LB_ANNOTATION 
