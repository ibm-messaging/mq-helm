...
export PORT="1414"
export IPADDRESS="$(kubectl get service multiinstance-ibm-mq-loadbalancer -o jsonpath='{..ip}')"

( echo "cat <<EOF" ; cat ccdt_template.json ; echo EOF ) | sh > ccdt_generated.json

echo "Starting amqsphac" multiinstance
/opt/mqm/samp/bin/amqsphac APPQ multiinstance
